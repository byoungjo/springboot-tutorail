...
//    private final StepBuilderFactory stepBuilderFactory; // 생성자 DI 받음
    ...
    @Bean
    @SneakyThrows
    public Job myBatisItemReaderJob(DataSource dataSource) throws Exception {
        log.info(">>>>>>>> myBatisItemReaderJob  ");
        Connection connection = dataSource.getConnection();
        log.info("DBCP: " + dataSource.getClass()); // 사용하는 DBCP 타입 확인
        log.info("Url: " + connection.getMetaData().getURL());
        log.info("UserName: " + connection.getMetaData().getUserName());

        return jobBuilderFactory.get("myBatisItemReaderJob") // job name
...
//        return writer;
    ...
    public PagingQueryProvider createQueryProvider() throws Exception {
        ...
        log.info(">>>>>>>> queryProviderFactoryBean ");
